...
|---------|---------|---------|------|
| **5) Tools & Infrastructure (Batches 3.0)** | 1 query | 1 text (header + profiles) + 3 counters + 1 table + 1 chart = **6-7** | 3-4 ряда |
...
РЯД S5-2: [3 КАУНТЕРА из Query 5.1]
          | Floe Lending Txs | FLOE Unique Users | FLOE DEX Volume |
...
| **Свой токен** | Нет |
| **Свой контракт** | Нет (использует x402 + USDC transfers) |
...
**Ончейн-следы:**
- Платежи проходят как USDC transfers на Base (и Solana)
- Нет dedicted контракта - используется стандартный USDC
...
| **Сеть** | Ethereum Sepolia, Base Sepolia, Polygon Amoy (тестнеты) |
| **Язык** | Rust (core), TypeScript + Python (SDKs) |
| **Свой токен** | Нет |
| **Контракт** | Только тестнеты (pre-alpha) |
...
**Ончейн-следы:**
- Пока только на тестнетах (Sepolia, Base Sepolia)
- Недавно добавлена поддержка Base Sepolia (commit от 12 апреля 2026)
- Активы: ETH, USDC, USDT
- **Не имеет mainnet deployment на момент V3** - информационное добавление
...
|---------|---------|---------|------|--------|
| Floe Labs (Lending) | `0x17946cD3e180f82e632805e5549EC913330Bb175` | Base Mainnet | Lending Protocol | **Active** |
| Floe Labs (FLOE Token) | `0xA2CBA6b88D86cC2F7469BbEabDd6b4A63c60f75e` | Base Mainnet | ERC-20 Token | **Active** (Apr 11, 2026) |
| ERC-8004 Registry | `0x8004A169FB4a3325136EB29fA0ceB6D2e539a432` | Base Mainnet | Identity Registry | **Active** (shared) |
| BlockRunAI | N/A | Base Mainnet | USDC via x402 | No dedicated contract |
| Agently | N/A | Base Mainnet | Uses ERC-8004 | No dedicated contract |
...
#### Что он делает
Отслеживает доступную ончейн-активность проектов Tools & Infrastructure на Base. Основной источник данных - Floe Labs (единственный проект с активными контрактами на mainnet). Также проверяет FLOE токен на DEX.

#### На какие вопросы отвечает
- Есть ли ончейн-активность у проектов Tools & Infrastructure?
- Сколько транзакций и пользователей у Floe Labs lending protocol?
- Какой объем торгов у FLOE токена?
- Какова динамика (WoW)?
...
-- V3: Tracks available on-chain metrics for Batches 003 projects
-- Primary data: Floe Labs (lending contract + FLOE token)
-- Note: BlockRunAI, Agently, 4mica have no dedicated mainnet contracts

WITH tool_contracts AS (
    SELECT address, name FROM (
        VALUES
        -- Floe Labs: Lending Protocol
...
-- Floe Labs DEX volume (FLOE token trades on Base)
floe_dex AS (
    SELECT
        ROUND(COALESCE(SUM(amount_usd), 0), 2) AS floe_volume_usd,
        COUNT(*) AS floe_trades,
        COUNT(DISTINCT taker) AS floe_traders
    FROM dex.trades
...
SELECT
    ...
    fd.floe_volume_usd AS "FLOE DEX Volume (USD)",
    fd.floe_trades AS "FLOE Trades",
    fd.floe_traders AS "FLOE Traders",
    CASE
    ...
    END AS "WoW Growth %"
FROM totals tot
CROSS JOIN floe_dex fd
CROSS JOIN wow_current wc
CROSS JOIN wow_prev wp
```

**Примечания:**
- Запрос покрывает только Floe Labs - единственный проект с mainnet контрактами
- BlockRunAI платежи проходят как обычные USDC transfers - невозможно отфильтровать без знания конкретных адресов кошельков
- Agently использует ERC-8004 (уже отслеживается в Section 5 дашборда - ERC-8004 Registry)
- 4mica еще не на mainnet
- Дата старта: 2026-04-01 (FLOE token deployed Apr 11)
...
-- Add FLOE DEX trades to the daily view
daily_dex AS (
    SELECT
...
SELECT
    COALESCE(ds.day, dd.day) AS day,
    COALESCE(ds.daily_txs, 0) AS "Contract Txs",
    COALESCE(ds.daily_users, 0) AS "Active Users",
    COALESCE(dd.dex_trades, 0) AS "FLOE DEX Trades",
    COALESCE(dd.dex_volume_usd, 0) AS "FLOE Volume (USD)"
FROM daily_stats ds
...
Из Query 5.1 создаем **3 Counter-визуализации**:

| # | Заголовок | Колонка | Формат | Цвет | Ширина |
|---|-----------|---------|--------|------|--------|
| 1 | **Total Transactions** | `Total Txs` | Number | Teal `#14B8A6` | 4 кол. |
| 2 | **Unique Users** | `Unique Users` | Number | Sky `#0EA5E9` | 4 кол. |
| 3 | **FLOE DEX Volume** | `FLOE DEX Volume (USD)` | USD, 0 десятичных | Green `#10B981` | 4 кол. |
...
| **Колонки** | Total Txs, Unique Users, FLOE DEX Volume (USD), FLOE Trades, WoW Growth % |
...
| **X-axis** | `day` |
| **Y-axis (left)** | `Contract Txs` (Teal `#14B8A6`) |
| **Y-axis (right)** | `FLOE Volume (USD)` (Green `#10B981`) |
...
| 4mica | Instant credit tabs (x402 facilitator) | Testnet only (pre-alpha) |

**Data limitations:**
- Only Floe Labs has dedicated mainnet contracts for direct tracking
- BlockRunAI payments are standard USDC transfers (indistinguishable without wallet mapping)
- Agently activity overlaps with ERC-8004 Registry section
- 4mica is not yet deployed on Base mainnet
...
- [ ] **Шаг 0.1:** Проверить актуальность контрактов:
  ...
  - FLOE Token: https://basescan.org/address/0xA2CBA6b88D86cC2F7469BbEabDd6b4A63c60f75e
...
- [ ] **Шаг 3:** Из Query 5.1 создать 3 Counter-визуализации:
  - Counter 1: Total Transactions (Teal)
  - Counter 2: Unique Users (Sky)
  - Counter 3: FLOE DEX Volume ($, Green)

- [ ] **Шаг 4:** Из Query 5.2 создать Line Chart:
  - X: day, Y-left: Contract Txs (Teal), Y-right: FLOE Volume (Green)
...
- [ ] **Шаг 6:** Разместить 3 каунтера (РЯД S5-2)
  - По 4 колонки каждый
...
- [ ] BlockRunAI: если появится dedicated контракт или API для получения статистики x402 платежей
- [ ] 4mica: когда запустится на Base mainnet - добавить Vault contract в `tool_contracts` CTE
