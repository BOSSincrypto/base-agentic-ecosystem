...
WITH owb_contracts AS (
    SELECT address, label, category FROM (
        ...
        (0x692d11c779d43BBC12Cb8565C7f71a54A47D117c, 'Staking', 'DeFi')
...
daily_filled AS (
    SELECT
        ...
        COALESCE(SUM(dc.daily_users), 0) AS contract_users,
        ...
        COALESCE(SUM(CASE WHEN dc.category = 'Agentic' THEN dc.daily_txs ELSE 0 END), 0) AS agentic_txs,
...
combined AS (
    SELECT
        ...
        df.contract_users AS "Active Users",
        df.token_txs AS "Token Txs",
        df.gaming_txs AS "Gaming (Claims) Txs",
        df.agentic_txs AS "Agentic Claims Txs",
        ...
        ROUND(AVG(df.contract_txs) OVER (ORDER BY df.day ROWS BETWEEN 6 PRECEDING AND CURRENT ROW), 0) AS "Contract Txs 7d MA",
        ROUND(AVG(COALESCE(dd.dex_volume_usd, 0)) OVER (ORDER BY df.day ROWS BETWEEN 6 PRECEDING AND CURRENT ROW), 2) AS "Volume 7d MA",
        SUM(df.contract_txs) OVER (ORDER BY df.day) AS "Cumulative Contract Txs",
        ROUND(SUM(COALESCE(dd.dex_volume_usd, 0)) OVER (ORDER BY df.day), 2) AS "Cumulative DEX Volume"
